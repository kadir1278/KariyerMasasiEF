async function GetData() {
    ...
    var url = '/departman-getir/' + searchText;
    $('#example').html("");
    var thead =
        '<thead class="thead-primary"><tr>' +
        '<th class="sorting_asc">Departman</th>' +
        '</tr></thead>';
...
$("#department-add-form").submit(function (e) {
    e.preventDefault();
    var btnClose = document.getElementById("closeUser");
    var user = {
        Name: $("#department-add-form").find('[name="AddName"]').val(),
    }
    console.log(user);
    $.ajax({
        type: "POST",
    ...
    bootbox.confirm("Silmek istediğinize emin misiniz ?", function (result) {
        if (result) {
            ...
            $.ajax({
                ...
                success: function () {
                    bootbox.alert("Departman silme işlemi başarılı.")
                    GetData();
                },
                error: function () {
                    bootbox.alert("Departman silinemedi lütfen tekrar deneyin. Sorunun devam etmesi durumunda 360MEKA ile irtibat kurunuz.");
                }
            })
        }
    })
