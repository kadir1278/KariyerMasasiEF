async function GetData() {
    ...
    var url = '/sirket-kullanicisi-getir/' + searchText;
    $('#example').html("");
    var thead =
...
$("#example").on("click", "#btnDelete", function () {
    ...
    bootbox.confirm("Silmek istediğinize emin misiniz ?", function (result) {
        if (result) {
            ...
            $.ajax({
                type: "GET",
                url: "/sirket-kullanicisi-sil/" + ID,
                success: function () {
                    bootbox.alert("Şirket kullanicisii silme işlemi başarılı.")
                    GetData();
                },
                error: function () {
                    bootbox.alert("Şirket kullanicisii silinemedi lütfen tekrar deneyin. Sorunun devam etmesi durumunda 360MEKA ile irtibat kurunuz.");
                }
            })
        }
    })
